...
func TestService_AccountLookupFailure_UsingMocks(t *testing.T) {
	...
	mkt.EXPECT().CurrentPrice(mock.Anything).Return(dec("42000"), nil)
	accounts.EXPECT().Balance(mock.Anything, "C001").Return(decimal.Zero, errors.New("db unavailable"))
...
func TestService_LedgerLookupFailure_UsingMocks(t *testing.T) {
	...
	mkt.EXPECT().CurrentPrice(mock.Anything).Return(dec("42000"), nil)
	accounts.EXPECT().Balance(mock.Anything, "C001").Return(dec("1000000"), nil)
	led.EXPECT().
		TradedToday(mock.Anything, "C001", mock.Anything).
