...
type Validator interface {
	Apply(ctx context.Context, o order.Order, res *order.Result) bool
}
...
func New(cfg Config, provider MarketPriceProvider) (Validator, error) {
	...
	return &validatorImpl{cfg: cfg, provider: provider}, nil
}

func (v *validatorImpl) Apply(ctx context.Context, o order.Order, res *order.Result) (marketOK bool) {
...
func (v *validatorImpl) checkBuy(o order.Order, market decimal.Decimal, res *order.Result) {
...
func (v *validatorImpl) checkSell(o order.Order, market decimal.Decimal, res *order.Result) {
...
// Returns false when expected is zero to avoid division-by-zero.
