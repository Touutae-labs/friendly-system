...
func TestModule_Validate(t *testing.T) {
	repo := testhelpers.NewAccountMemory(map[string]decimal.Decimal{
		"C001": d("1000"),
	})
	m, _ := balance.New(repo)
	...
	tests := []struct {
		...
		{name: "valid buy", customerID: "C001", qty: "1", price: "1000", wantError: ""},
		{name: "insufficient balance", customerID: "C001", qty: "1.1", price: "1000", wantError: balance.CodeInsufficientBalance},
		{name: "customer not found", customerID: "GHOST", qty: "1", price: "100", wantError: balance.CodeCustomerNotFound},
	}
...
func TestModule_Apply(t *testing.T) {
	...
	t.Run("buy success", func(t *testing.T) {
		bal, err := m.Apply(context.Background(), order.Order{
			CustomerID:  "C001",
			Quantity:    d("0.5"),
			QuotedPrice: d("1000"),
			OrderType:   order.Buy,
