...
func mustNew(
	t testing.TB,
	accounts *testhelpers.AccountMemory,
	mkt quote.MarketPriceProvider,
	led *testhelpers.LedgerMemory,
) validatorsvc.ValidatorService {
	t.Helper()
...
func fixtures(t *testing.T, marketPrice string) (validatorsvc.ValidatorService, *testhelpers.LedgerMemory) {
	t.Helper()
	accounts := testhelpers.NewAccountMemory(map[string]decimal.Decimal{
		"C001": dec("1000000"),
		"C002": dec("500"),
	})
	mkt := quote.NewFixed(dec(marketPrice))
...
func TestMarketUnavailableFailsClosed(t *testing.T) {
	accounts := testhelpers.NewAccountMemory(map[string]decimal.Decimal{"C001": dec("1000000")})
	led := testhelpers.NewLedgerMemory()
	v := mustNew(t, accounts, quote.NewFixed(dec("0")), led)
...
// TestService_AccountLookupFailure uses WithError to simulate an infrastructure
// failure without needing generated mocks.
func TestService_AccountLookupFailure(t *testing.T) {
	accounts := testhelpers.NewAccountMemory(nil).WithError(errors.New("db unavailable"))
	led := testhelpers.NewLedgerMemory()
	v := mustNew(t, accounts, quote.NewFixed(dec("42000")), led)
	r := v.Validate(context.Background(), order.Order{
		CustomerID:  "C001",
		OrderType:   order.Buy,
		Quantity:    dec("1"),
...
func TestService_LedgerLookupFailure(t *testing.T) {
	accounts := testhelpers.NewAccountMemory(map[string]decimal.Decimal{"C001": dec("1000000")})
	led := testhelpers.NewLedgerMemory().WithError(errors.New("redis timeout"))
	v := mustNew(t, accounts, quote.NewFixed(dec("42000")), led)
	r := v.Validate(context.Background(), order.Order{
		CustomerID:  "C001",
		OrderType:   order.Buy,
...
func TestDailyLimitExceededAfterPriorOrders(t *testing.T) {
	...
	led.Record("C001", dec("4.5"), fixedNow)
